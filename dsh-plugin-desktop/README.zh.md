...
插件作者应遵循 [Desktop 插件 service 架构](docs/plugin-services.zh.md)中记录的受支持 contract import、生命周期规则与适配模式。IM 配对、局域网手机 PWA、进程内免token 网关与目录式插件市场见 [Desktop 频道](docs/channels.zh.md) 和 [Desktop 插件市场](docs/marketplace.zh.md)。
...
在 Windows 上，Launcher 会固定使用 desktop 自有的 browse 目录选择 backend 与配套 client surface，而不使用自适应 native chooser。因此 workspace 选择始终在 Web UI 内完成，也不会在 Electron main 进程中加载原生 N-API 对话框 worker。Windows 的 Home 列表还会列出 `D:\` 等其他盘符根目录，因为应用内面包屑停在用户目录、无法跨盘。macOS 与 Linux 仍使用上游自适应 chooser。
...
# yarn dev：用 PATH 里那个 yarn。如果本机装过 Yarn 1（yarn -v 是 1.x），它可能根本对不上这个仓库。
# corepack yarn dev：走 Node 自带的 Corepack，按根目录 package.json 里的 "packageManager": "yarn@4.18.0" 启动 Yarn 4.18.0，不依赖全局装过什么 Yarn。
```
...
打包后的 macOS 与 Windows 应用会在启动 5 秒后查询 `https://github.com/hyqibot/DeepSeek-Harness-Token-Free/releases/latest/download/latest.json`，并在每次检查完成六小时后再次查询。每次 no-cache 请求会跟随 GitHub 重定向，期限为 15 秒，并与托盘中的 **Check for Updates…** 命令共用一个 in-flight operation。响应只有在包含 `{ "version": "x.y.z" }` 形式的规范 stable Semantic Versioning 时才会被接受。后台检查遇到网络、HTTP、超时、无效响应、相同版本或服务端旧版本时保持静默。后台发现新版本时会发出系统通知，并把确认对话框绑到已还原的主窗口上。手工检查一定会显示原生结果对话框：相同或旧版本会显示当前安装版本，失败会提示用户重试，严格更新的版本则显示 **立即下载** 或 **稍后**。自动更新提示会按版本记录，用户仍可从托盘显式重试。开发运行、未打包启动与 Linux 不会下载安装包。

选择 **Download** 后，应用会先重新确认 `latest.json` 中的版本没有变化，然后请求对应 GitHub Release 资源（Windows 为 `DSH-Desktop-{version}-x64-Setup.exe`，macOS 为 `DSH-Desktop-{version}-{arm64|x64}.dmg`）。DSH Desktop 使用 Electron 网络跟随 redirect，把不超过 1 GiB 的文件流式写入私有、按版本划分的 user-data 目录，并在交付前拒绝不完整的 DMG 或 Windows PE。macOS 会打开下载好的 DMG，并提示用户替换 `Applications` 中的应用后重新打开。当前 CI 的 macOS 包未签名、未公证，Gatekeeper 可能警告，用户可在 Finder 中右键选择 **打开**。Windows 会在 NSIS 安装器准备完成后再次确认；选择 **Restart and Install** 会启动安装器，并在当前进程退出前请求 Cordis 有序 teardown。下载、文件系统与安装器打开失败都会保持静默，同时保留托盘中的可重试版本操作。

维护者必须先发布两个平台安装包，再上传 `latest.json`。私有仓 **Release Desktop** 会在 `v*.*.*` tag 上同时构建 Windows 与 macOS，最后才发布 `latest.json`，避免客户端发现一个还没有安装包的版本。
...
`dist:win` 会拒绝非 Windows 或非 x64 宿主，先执行一组 Windows 可运行的 gate，其中包括 build、全部 TypeScript compiler face、打包与原生 shell 聚焦测试，以及 runtime-closure verifier；随后再构建 NSIS 安装向导，并校验生成的两个 PE 文件。完整跨平台 suite 仍由 CI 持有，因为其中部分 POSIX 执行测试不是 Windows 程序。安装向导支持当前用户安装或提升权限后的所有用户安装，可更改安装目录，会创建开始菜单与桌面快捷方式，并且卸载应用时保留 DSH 用户数据。版本 `2.2.5` 会输出到 `dsh-plugin-desktop\dist\DSH-Desktop-2.2.5-x64-Setup.exe`；用于 smoke 测试的未封装程序仍位于 `dsh-plugin-desktop\dist\win-unpacked\DSH Desktop.exe`。
