...
describe('public Desktop version check', () => {
  it('uses only the fixed no-cache version endpoint and reports a newer stable version', async () => {
    ...
    expect(calls[0]?.url).toContain('hyqibot/DeepSeek-Harness-Token-Free')
    expect(calls[0]?.url).toContain('/releases/latest/download/latest.json')
    expect(calls[0]?.url).not.toContain('dshdesktop.cn')
