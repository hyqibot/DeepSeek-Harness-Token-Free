...
describe('desktop terminal environment', () => {
  ...
  it('generates private macOS shims and opens one quoted welcome command', () => {
    ...
    expect(welcome).toContain('DeepSeek-Harness-Token-Free 2.0.0 terminal')
    ...
    expect(welcome).toContain('Restart DeepSeek-Harness-Token-Free after plugin changes.')
  ...
  it('generates Windows batch shims and opens PowerShell through a visible-console broker', () => {
    ...
    expect(welcome).toContain('"DeepSeek-Harness-Token-Free {0} terminal" -f $env:DSH_DESKTOP_PRODUCT_VERSION')
    ...
    expect(launcher).toContain('start "DeepSeek-Harness-Token-Free" /D "!DSH_DESKTOP_PROFILE_DIRECTORY!"')
