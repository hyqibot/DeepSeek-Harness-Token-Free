...
describe('desktop update Host plugin', () => {
  it('exposes the packaged 5-second and six-hour background policy', () => {
  ...
  it('prompts once for a background update and persists only state v2 prompt history', async () => {
    ...
    expect(harness.downloadAndOpen).not.toHaveBeenCalled()
    expect(harness.tray.label()).toBe('DeepSeek-Harness-Token-Free 2.1.0 Available')
    ...
    expect(harness.notifications).toEqual([{
      title: '发现新版本',
      body: 'DeepSeek-Harness-Token-Free 2.1.0 已发布，请下载更新。',
    }])
    expect(harness.warnings).toEqual([])
  })
  ...
  it('downloads and opens only after confirmation', async () => {
    ...
    expect(harness.tray.label()).toBe('Downloading DeepSeek-Harness-Token-Free 2.1.0…')
    expect(harness.notifications).toEqual([{
      title: '发现新版本',
      body: 'DeepSeek-Harness-Token-Free 2.1.0 已发布，请下载更新。',
    }])
    ...
    resolveDownload()
    await vi.waitFor(() => { expect(harness.tray.label()).toBe('DeepSeek-Harness-Token-Free 2.1.0 Available') })
    expect(harness.notifications).toEqual([{
      title: '发现新版本',
      body: 'DeepSeek-Harness-Token-Free 2.1.0 已发布，请下载更新。',
    }])
  ...
  it('rechecks the version after confirmation and skips a rotated download', async () => {
    ...
    expect(harness.downloadAndOpen).not.toHaveBeenCalled()
    expect(harness.showManualCheckResult).not.toHaveBeenCalled()
    expect(harness.tray.label()).toBe('DeepSeek-Harness-Token-Free 2.2.0 Available')
  })
