...
describe('Electron compatibility runtime', () => {
  ...
  it('uses Electron networking and confirmation-gated macOS update handoff', async () => {
    ...
    expect(electron.dialog.showMessageBox).toHaveBeenLastCalledWith(expect.objectContaining({
      title: 'DeepSeek-Harness-Token-Free Is Up to Date',
    ...
    expect(electron.dialog.showMessageBox).toHaveBeenLastCalledWith(expect.objectContaining({
      title: '发现新版本',
      message: 'DeepSeek-Harness-Token-Free 2.1.0 已发布',
      buttons: ['立即下载', '稍后'],
      defaultId: 0,
    }))
    ...
    expect(electron.dialog.showMessageBox).toHaveBeenLastCalledWith(expect.objectContaining({
      title: 'DeepSeek-Harness-Token-Free Update Downloaded',
      buttons: ['OK'],
    }))
  ...
  it('parents the update confirmation dialog on the restored window', async () => {
    vi.spyOn(process, 'platform', 'get').mockReturnValue('win32')
    const { ElectronDesktopRuntime } = await import('../src/electron-runtime.ts')
    const runtime = new ElectronDesktopRuntime(async () => {})
    ...
    if (window === undefined) throw new Error('expected the scheduled window')
    window.isMinimized.mockReturnValue(true)

    await expect(runtime.updates.confirmDownload('2.2.1')).resolves.toBe(true)
    ...
    expect(electron.dialog.showMessageBox).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        title: '发现新版本',
        message: 'DeepSeek-Harness-Token-Free 2.2.1 已发布',
        buttons: ['立即下载', '稍后'],
      }),
    )
  })
