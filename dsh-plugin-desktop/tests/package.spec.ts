...
describe('published package surface', () => {
  ...
  it('exposes the Host plugin and desktop-owned client face', () => {
    ...
    expect(manifest.exports).toHaveProperty('./channels', {
      types: './lib/types/channels.d.ts',
      default: './lib/channels.js',
    })
    expect(manifest.exports).toHaveProperty('./mobile', {
      types: './lib/types/mobile.d.ts',
      default: './lib/mobile.js',
    })
    expect(manifest.exports).toHaveProperty('./zero-token', {
      types: './lib/types/zero-token.d.ts',
      default: './lib/zero-token.js',
    })
    expect(manifest.exports).toHaveProperty('./hyqi', {
      types: './lib/types/hyqi.d.ts',
      default: './lib/hyqi.js',
    })
    expect(manifest.exports).toHaveProperty('./marketplace', {
      types: './lib/types/marketplace.d.ts',
      default: './lib/marketplace.js',
    })
    expect(manifest.exports).toHaveProperty('./plugin-toggles', {
      types: './lib/types/plugin-toggles.d.ts',
      default: './lib/plugin-toggles.js',
    })
    ...
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: dsh-plugin-desktop/channels')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: dsh-plugin-desktop/mobile')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: dsh-plugin-desktop/zero-token')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: dsh-plugin-desktop/hyqi')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: dsh-plugin-desktop/marketplace')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: dsh-plugin-desktop/plugin-toggles')
  })
  ...
  it('builds public Host plugins and their private native bootstraps', () => {
    ...
    expect(config).toContain("channels: 'src/channels.ts'")
    expect(config).toContain("mobile: 'src/mobile.ts'")
    expect(config).toContain("'zero-token': 'src/zero-token.ts'")
    expect(config).toContain("hyqi: 'src/hyqi.ts'")
    expect(config).toContain("marketplace: 'src/marketplace.ts'")
    expect(config).toContain("'plugin-toggles': 'src/plugin-toggles.ts'")
  })
  ...
  it('fixes the installed application identity', () => {
    ...
    expect(manifest.build?.productName).toBe('DeepSeek-Harness-Token-Free')
    ...
    expect(manifest.build?.dmg?.artifactName).toBe('DSH-Desktop-${version}-${arch}.${ext}')
    ...
    expect(manifest.build?.nsis).toEqual({
      ...
      shortcutName: 'DeepSeek-Harness-Token-Free',
  ...
  it('separates unsigned smoke packaging from the signed macOS release', () => {
    ...
    expect(manifest.scripts?.['dist:mac:ci']).toBe('node scripts/package-mac-ci.ts')
    expect(manifest.scripts?.['dist:win']).toBe('node scripts/package-win.ts')
    expect(manifest.scripts?.['check:mac-package']).toContain('tests/package-mac-ci.spec.ts')
    expect(manifest.scripts?.['check:mac-package']).toContain('tests/update-checker.spec.ts')
    ...
    expect(workspaceManifest.scripts?.['dist:mac:ci'])
      .toBe('yarn workspace dsh-plugin-desktop dist:mac:ci')
