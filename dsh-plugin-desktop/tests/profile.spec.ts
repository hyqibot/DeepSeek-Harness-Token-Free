...
describe('desktop profile composition', () => {
  ...
  it('assembles the Host shell without replacing the upstream client shell', { timeout: 20_000 }, () => {
    const home = temporaryHome()
    ...
    expect(rows.find(row => row.id === 'desktop-channels')).toEqual(expect.objectContaining({
      name: 'dsh-plugin-desktop/channels',
    }))
    expect(rows.find(row => row.id === 'desktop-mobile')).toEqual(expect.objectContaining({
      name: 'dsh-plugin-desktop/mobile',
    }))
    expect(rows.find(row => row.id === 'desktop-zero-token')).toEqual(expect.objectContaining({
      name: 'dsh-plugin-desktop/zero-token',
    }))
    expect(rows.find(row => row.id === 'desktop-hyqi')).toEqual(expect.objectContaining({
      name: 'dsh-plugin-desktop/hyqi',
    }))
    expect(rows.find(row => row.id === 'desktop-marketplace')).toEqual(expect.objectContaining({
      name: 'dsh-plugin-desktop/marketplace',
    }))
    expect(rows.find(row => row.id === 'desktop-plugin-toggles')).toEqual(expect.objectContaining({
      name: 'dsh-plugin-desktop/plugin-toggles',
    }))
  ...
  it('pins the Windows browse picker and desktop pwsh provider without replacing process boundaries', () => {
    ...
    expect(rows).toContainEqual(expect.objectContaining({
      ...
      name: 'dsh-plugin-desktop/directory-picker-browse',
    }))
    expect(rows).toContainEqual(expect.objectContaining({
  ...
  it('applies plugin disable overlays after normalizing legacy include-prefixed ids', () => {
    const home = temporaryHome()
    const dir = ensureDesktopProfile(home)
