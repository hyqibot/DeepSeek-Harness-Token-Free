...
本文档是面向插件作者、受支持的 Host 侧集成 contract，覆盖 DSH Desktop 2.x 在兼容与高级两种呈现模式下导出的公开 `desktopProfiles`、`desktopPnpm` 与 `desktopChannels` Cordis service。它不会授予第三方访问原始 Electron API、renderer 或 launcher bootstrap 状态的能力。
...
| `desktopChannels` | 作用于 generation 的 IM 配对存储与 Agent 分发器。 | 公开；通过 `dsh-plugin-desktop/channels`。`desktop-mobile` 会使用它。 |
