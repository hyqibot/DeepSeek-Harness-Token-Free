...
This document is the supported Host-side integration contract for plugin authors. It covers the public `desktopProfiles`, `desktopPnpm`, and `desktopChannels` Cordis services exported by DSH Desktop 2.x in both compatibility and advanced presentation modes. It does not grant third-party access to raw Electron APIs, the renderer, or launcher bootstrap state.
...
| `desktopChannels` | Generation-scoped IM pairing store and Agent dispatcher. | Public through `dsh-plugin-desktop/channels`. Used by `desktop-mobile`. |
...
The fact that a private type is present in emitted declarations does not make its runtime service a supported third-party capability. The public service names and their contract modules are the compatibility boundary.
