...
import { normalizePatchLayerIds } from './plugin-toggles-patch.ts'
...
const BROWSE_PICKER_BACKEND = 'dsh-plugin-desktop/directory-picker-browse'
...
export function prepareDesktopProfile(
  ...
  const homePatches = normalizePatchLayerIds(loadOptionalPatches(BIN_NAME, join(home, PROFILE_PATCH_FILENAME)) ?? [])
  const patches: PatchOptions[] = [
    ...bundlePatches,
    ...normalizePatchLayerIds(profile.patches),
    ...homePatches,
  ]
