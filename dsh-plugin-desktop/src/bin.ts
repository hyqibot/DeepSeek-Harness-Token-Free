...
import { decodeChromiumConsoleChunk } from './windows-chromium-log.ts'
...
/**
 * On Windows, Chromium logs UTF-8 through a GBK console and the crashpad
 * CreateFile line becomes mojibake. Pipe the streams and rewrite them as UTF-8.
 */
export function forwardElectronConsole(child: ChildProcess): void {
  const write = (chunk: Buffer, dest: NodeJS.WritableStream): void => {
    dest.write(decodeChromiumConsoleChunk(chunk))
  }
