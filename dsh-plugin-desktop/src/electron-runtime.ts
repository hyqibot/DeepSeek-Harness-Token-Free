...
export class ElectronDesktopRuntime implements DesktopRuntime {
  ...
  private async confirmUpdateDownload(version: string): Promise<boolean> {
    const result = await this.showAppMessageBox({
      type: 'info',
      title: '发现新版本',
      message: `${PRODUCT_NAME} ${version} 已发布`,
      detail: '是否立即下载安装包？',
      buttons: ['立即下载', '稍后'],
      defaultId: 0,
      cancelId: 1,
  ...
  /**
   * Bring the desktop window forward so an update prompt cannot hide behind it.
   * @param options - native message-box options.
   */
  private async showAppMessageBox(
    options: Electron.MessageBoxOptions,
  ): Promise<Electron.MessageBoxReturnValue> {
    const parent = this.visibleUpdateWindow()
    return parent === undefined
      ? dialog.showMessageBox(options)
      : dialog.showMessageBox(parent, options)
  }

  /** Restore and focus the live BrowserWindow, or undefined when none exists. */
  private visibleUpdateWindow(): BrowserWindow | undefined {
    const window = this.window
