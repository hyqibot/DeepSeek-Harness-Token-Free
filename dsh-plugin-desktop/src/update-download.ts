...
import { PUBLIC_RELEASES_REPO, parseSemVer } from './update-checker.ts'

/** Desktop platforms with a GitHub Release installer. */
export type DesktopDownloadPlatform = 'darwin' | 'win32'

/**
 * Public GitHub Release asset URL for one confirmed installer download.
 * @param platform - Host platform selecting NSIS vs DMG.
 * @param version - Canonical stable SemVer already advertised by latest.json.
 * @param arch - CPU architecture; Windows always uses the x64 NSIS installer.
 */
export function desktopDownloadUrl(
  ...
  const base = `https://github.com/${PUBLIC_RELEASES_REPO}/releases/download/v${version}`
  if (platform === 'win32') return `${base}/DSH-Desktop-${version}-x64-Setup.exe`
  const macArch = arch === 'x64' ? 'x64' : 'arm64'
  return `${base}/DSH-Desktop-${version}-${macArch}.dmg`
}
