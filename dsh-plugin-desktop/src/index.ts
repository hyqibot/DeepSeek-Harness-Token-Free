...
import { PRODUCT_NAME, WINDOW_TITLE } from './product.ts'
import {
  dispatchShellHttp,
  SHELL_HTTP_PREFIX,
} from './shell-http.ts'
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.effect(() => {
    const unregisterHttp = ctx.webServer.register({
      kind: 'prefix',
      path: SHELL_HTTP_PREFIX,
      handler: (req, res) => {
        void dispatchShellHttp(req, res, {
          snapshot: () => ({
            mode: settings.get().mode,
            platform: ctx.desktopRuntime.platform,
            advancedSupported: ctx.desktopRuntime.platform !== 'linux',
          }),
          async setMode(mode) {
  ...
  }, 'dsh-plugin-desktop: shell settings HTTP')
  ctx.effect(() => {
