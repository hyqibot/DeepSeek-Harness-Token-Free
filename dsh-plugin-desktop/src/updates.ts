...
export const Config: z<Config> = z.object({
  ...
  initialDelayMs: z.number().step(1).min(0).max(MAX_TIMER_DELAY_MS).default(5_000),
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.effect(() => {
    ...
    let lastResult: UpdatesStatusPayload['lastResult'] = 'idle'
    ...
    const observeResult = (result: UpdateCheckResult | null): string | undefined => {
      ...
      lastResult = result.status === 'update-available' ? 'available' : 'up-to-date'
    ...
    const startDownload = (version: string, confirm = true): Promise<void> => {
    ...
    const offerDownload = async (version: string, automatic: boolean): Promise<void> => {
      ...
      if (automatic) {
        adapter.notify({
          title: '发现新版本',
          body: `${PRODUCT_NAME} ${version} 已发布，请下载更新。`,
        })
      }
    ...
    const unregisterHttp = ctx.webServer.register({
      kind: 'prefix',
      path: UPDATES_HTTP_PREFIX,
      handler: (req, res) => {
        void dispatchUpdatesHttp(req, res, {
          snapshot,
          async check() {
            const result = await startCheck()
          ...
          async download() {
            if (availableVersion !== undefined) await startDownload(availableVersion, false)
            return snapshot()
          },
        })
      },
